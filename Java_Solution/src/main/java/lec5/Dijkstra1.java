...
public class Dijkstra1 {
    ...
    public Dijkstra1(String file) throws FileNotFoundException {
    ...
    private void relaxWeight(int u, int v, int w, PriorityQueue<int[]> minHeap) {
        ...
        if (distanceToSource.get(v) > distanceToSource.get(u) + w) {
            ...
            minHeap.add(new int[]{distanceToSource.get(v), v});
        }
    }
    ...
    public void calcShortestPath() {
        ...
        minHeap.add(new int[]{distanceToSource.get(1), 0});


        while (minHeap.size() > 0) {

            Integer u = minHeap.poll()[1];

            for (int[] vs : vertices.get(u)) {
                int v = vs[0];
                int w = vs[1];
                relaxWeight(u, v, w, minHeap);
            }
        }
    }

    public static void main(String[] args) throws FileNotFoundException {
        Dijkstra dijkstra = new Dijkstra("dijkstraData.txt");
